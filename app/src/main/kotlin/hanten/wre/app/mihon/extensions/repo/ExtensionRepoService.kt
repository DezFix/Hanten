package hanten.wre.app.mihon.extensions.repo
...
import hanten.wre.app.core.prefs.GitHubMirror
import hanten.wre.app.mihon.MihonExtensionLoader
...
class ExtensionRepoService @Inject constructor(
	...
	suspend fun fetchRepoDetails(baseUrl: String, type: ExternalExtensionType): ExternalExtensionRepo {
		if (type == ExternalExtensionType.IREADER || type == ExternalExtensionType.JAR) {
			...
			val derived = deriveRepoName(baseUrl, if (type == ExternalExtensionType.IREADER) "IReader" else "Hanten")
			val repoName = if (type == ExternalExtensionType.IREADER) "IReader: $derived" else "Hanten: $derived"
