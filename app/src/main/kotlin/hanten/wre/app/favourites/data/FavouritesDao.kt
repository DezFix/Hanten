package hanten.wre.app.favourites.data
...
import hanten.wre.app.core.db.MangaQueryBuilder
...
import hanten.wre.app.list.domain.ReadingProgress.Companion.PROGRESS_COMPLETED
...
abstract class FavouritesDao : MangaQueryBuilder.ConditionCallback {
	...
	@Query("SELECT manga.source AS count FROM favourites LEFT JOIN manga ON manga.manga_id = favourites.manga_id WHERE favourites.deleted_at = 0 AND (SELECT show_in_lib FROM favourite_categories WHERE favourite_categories.category_id = favourites.category_id) = 1 GROUP BY manga.source ORDER BY COUNT(manga.source) DESC LIMIT :limit")
