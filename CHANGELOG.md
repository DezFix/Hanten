...
### Трекеры
- Shikimori: оценка наконец ставится и не слетает — убрано зацикливание запросов, обновления идут последовательно
- Shikimori, MAL, AniList: удаление из приложения теперь удаляет запись и на сайте, с подтверждением
- MAL: починен refresh токена, оценка округляется правильно, комментарий больше не затирается
- Экспорт библиотеки в Shikimori больше не затирает оценку на сайте при сбое сети и отправляет прочитанные главы

### Статистика
- Время в сводке считается верно: записи больше не приходят не по порядку, финальное время сессии не теряется
- В режиме «по жанрам» итог больше не завышается в несколько раз, страницы в сводке учитывают фильтр по категориям
- Деление на ноль в процентах и в диаграммах заменено на безопасные значения
- Сводка: значки вместо слов, компактные значения (12ч 30м, &lt;1м), битая вёрстка длинных названий в списке

### Онбординг и настройки
- Экран приветствия больше нельзя закрыть случайно, но и заблокировать нельзя: кнопка «Готово» убрана, лист закрывается как все остальные
- Источники не могут остаться выключены все: если снять все галочки, включаются все манга-источники
- Выбор вида библиотеки на экране приветствия заработал
- Telegram-бэкапы: свой бот вместо общего, токен больше не вшит в сборку

### Стабильность
- Утечка: список источников больше не удерживается незавершённой загрузкой иконки
- Загрузки: очередь не теряется после сбоя предыдущей, чужие временные файлы не удаляются
- История: записи не откатываются назад, обновления трекера больше не держат блокировку базы на время сети
- Ссылки и QR: открытая по ссылке манга теперь показывает избранное, историю и трекер
